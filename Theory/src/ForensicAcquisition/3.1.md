...
\includegraphics[width=0.9 \linewidth]{./media/3.1.0.png}
\end{wrapfigure}
...
\begin{center}
\includegraphics[width=0.7 \linewidth]{./media/3.1.1.png}
\end{center}
...
sudo partx -a /dev/loop1 # Since the loop device represents an entire disk, Linux does not automatically recognize partitions. Use partx to make them available
...
\includegraphics[width=0.9 \linewidth]{./media/3.1.2.png}
\end{wrapfigure}
...
sudo dd if=/dev/loop1 bs=512 | tee image.dd | hashdeep -c md5,sha1 > image.src_hash # #apt install hashdeep
...
* **Expert Witness Format (EWF)**
    ...
    * ```ewfexport```: exports storage media data in EWF files to (split) RAW format or a specific version of
    EWF files.
    * sudo systemctl restart guymager
